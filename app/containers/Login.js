...
class Login extends Component {
  ...
  onLogin = () => {
    ...
    if(/^[\u3220-\uFA29]+$/.test(this.state.domain)){
      Toast.show("域名请输入英文字符", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    if(/^[\u3220-\uFA29]+$/.test(userName)){
      Toast.show("账号请输入英文字符", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    if(!this.state.domain){
      Toast.show("域名不可空！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    if(!userName){
      Toast.show("账号不可空！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    if(!password){
      Toast.show("密码不可空！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
      ...
      <View style={fetching ? styles.loading_container : styles.container}>
        ...
        >
          ...
          <View style={styles.content_container}>
            <KeyboardAvoidingView behavior='padding' style={styles.content_container}>
