...
export default class AddPage extends Component {
  ...
  loadStorageProject = (value) =>{
    ...
    this.props.dispatch({
      ...
      if (createmeta && createmeta.projects.length > 0) {
        ...
        try {
          createmeta.projects[0].issuetypes[0].fields.fixVersions.allowedValues.forEach((item) => {
            if (!item.released) {
          ...
          Toast.show("该项目没有可指定的解决版本", {duration: Toast.durations.LONG, position: Toast.positions.CENTER,})
        }
        const customItems = []
        const customCreatemeta = createmeta.projects[0].issuetypes[0].fields
        for (const item in customCreatemeta) {
          if (customCreatemeta[item].required && customCreatemeta[item].allowedValues && item.indexOf("customfield") > -1) {
            const formateData = []
        ...
        customItems.forEach((item) => {
          subCustomItems.push([item.customfieldId, item.customfieldValues[0].value])
        })
        this.setState({
          fixVersions: formatFixVersions.length > 0 ? formatFixVersions : null,
          projectData: value,
        ...
        Loading.hidden()
        Toast.show("该项目信息异常，请检查配置！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      }
    })
  }
      ...
      this.props.dispatch({
        ...
        if (assigneeList) {
          ...
          this.setState({
            ...
            assigneeName: formatData.length > 0 ? [formatData[0].value] : null
          })
        }
      })
    } else {
      Toast.show("请先选择项目！", {duration: Toast.durations.LONG, position: Toast.positions.CENTER,})
    }
  }

  checkProject=()=>{
    if (!this.state.projectData) {
      Toast.show("请先选择项目！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
  }
  ...
  handleSubmit=()=>{
    ...
    if(!this.state.summary){
      Toast.show("标题不可为空！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    if(!this.state.description){
      Toast.show("描述不可为空！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    if(!this.state.assigneeName){
      Toast.show("请指定指给谁!", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      return
    }
    let BaseParams ={}
    BaseParams = {
    ...
    this.props.dispatch({
        ...
        this.state.files.forEach((item)=>{
          ...
          this.props.dispatch({
            ...
            if(count === filesLength){
              ...
              Loading.hidden()
              this.props.dispatch(NavigationActions.navigate({routeName: 'Home',params:{updateHome:true,fetchDomain:this.props.domain}}))
            }
          })
        })
      }else {
      ...
      Toast.show("提交失败！", {duration: Toast.durations.SHORT, position: Toast.positions.CENTER,})
      Loading.hidden()
    })
  }
      ...
      <View style={styles.container}>
        ...
        >
          <List renderHeader={() => '基本信息'}>
            ...
            <TextInput
              style={{marginLeft: 10, borderBottomColor: "#eef3f5", borderBottomWidth: 1,fontSize:16}}
              placeholder='请输入标题'
              numberOfLines={1}
          ...
          {this.state.customItems && <List renderHeader={() => 'custom Field'}>
          ...
          <List renderHeader={() => 'Assignee'}>
            ...
            <Picker
              ...
              value={!this.state.assigneeName && this.state.assigneeList.length > 0 ? [this.state.assigneeList[0].value] : this.state.assigneeName}
